...
def tf(fichier):
    ...
    with open(fichier, 'r', encoding="utf-8") as f:
        ...
        for ligne in f:
            ...
            for mot in mots:
                # Si le mot est déjà une clé dans le dictionnaire, incrémenter sa valeur
                if mot in occurrences:
                    occurrences[mot] += 1
                # Sinon, ajouter le mot comme nouvelle clé avec une valeur de 1
                else:
                    occurrences[mot] = 1

    # Retourner le dictionnaire des occurrences
    return occurrences

print(tf("cleaned\\Nomination_Chirac1.txtcleaned.txt")["elle"])
...
def calculer_tfidf(repertoire):
    ...
    liste_mot_finale=list(set_mots)
    listemots = list(set_mots)
    liste_inter = [0] * len(files_names)
    matrice = [liste_inter] * len(set_mots)
...
#calculer_tfidf("cleaned\\")

#print(calculer_tfidf("cleaned\\"))